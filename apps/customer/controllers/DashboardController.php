...
class DashboardController extends Controller
{
    ...
    public function actionGlance()
    {
        ...
        $lists = Lists::model()->countByAttributes(array('customer_id' => $customer_id));
        ...
        $criteria->with = array(
            'list'  => array(
                ...
                'together'  => true,
                'joinType'  => 'INNER JOIN',
                'condition' => 'list.customer_id = :customer_id',
    ...
    public function actionSubscribers_growth()
    {
        ...
        $criteria->with = array(
            'list' => array(
                ...
                'joinType'  => 'INNER JOIN',
                'condition' => 'list.customer_id = :customer_id',
                'params'    => array(':customer_id' => (int)$customer_id)
            ),
        );
    ...
    public function actionLists_growth()
    {
        ...
        $criteria = new CDbCriteria();
        $criteria->select    = 'DISTINCT(DATE(t.date_added)) AS date_added';
        $criteria->condition = 't.customer_id = :customer_id AND DATE(t.date_added) >= DATE_ADD(LAST_DAY(DATE_SUB(NOW(), INTERVAL 3 MONTH)), INTERVAL 1 DAY)';
        $criteria->group     = 'MONTH(t.date_added)';
        $criteria->order     = 't.date_added ASC';
        $criteria->limit     = 3;
        $criteria->params    = array(':customer_id' => $customer_id);
        ...
        foreach ($models as $model) {
            $criteria = new CDbCriteria();
            $criteria->condition = 'customer_id = :customer_id AND YEAR(t.date_added) = YEAR(:year) AND MONTH(t.date_added) = MONTH(:month)';
            $criteria->params = array(
