...
class TransactionalEmail extends ActiveRecord
{
	...
	public function rules()
	{
		$rules = array(
			...
			array('customer_id, to_email, to_name, from_email, from_name, reply_to_email, reply_to_name, subject, status', 'safe', 'on'=>'search'),
		);
	...
	public function search()
	{
        ...
        if (!empty($this->customer_id) && is_string($this->customer_id)) {
            $criteria->with['customer'] = array(
                'together' => true,
                'joinType'  => 'INNER JOIN',
                'condition' => '(CONCAT(customer.first_name, " ", customer.last_name) LIKE :name OR email LIKE :name)',
                'params'    => array(
